...
# print(f"Getting top songs of {DECADE}...")
...
for YEAR in range(DECADE, min(DECADE+10, 2025), 1):
    print(f"Getting top songs of {YEAR}...")
    df = pd.read_csv(f"./combinedCharts/charts_{YEAR}_combined.txt", sep=";", encoding="cp1252")
    if not os.path.exists(f"./songs/songs{YEAR}"):
        os.makedirs(f"./songs/songs{YEAR}")
        print(df.head())
        ...
        for title, artist in zip(df["title"], df["artist"]):
            ...
            if not os.path.exists(f"./songs/songs{YEAR}/{title} by {artist}.txt"):
                ...
                try:
                    ...
                    pass
                try:
                    with open(f"./songs/songs{YEAR}/{title} by {artist}.txt", "w") as f:
                        f.write(unicodedata.normalize("NFKD", song.lyrics).encode("ascii", "ignore").decode("utf-8"))
                except AttributeError:
                    os.remove(f"./songs/songs{YEAR}/{title} by {artist}.txt")
                    ...
                    try:
                        ...
                        pass
                    try:
                        with open(f"./songs/songs{YEAR}/{title} by {artist} UNCERTAIN.txt", "w") as f:
                            f.write(unicodedata.normalize("NFKD", song.lyrics).encode("ascii", "ignore").decode("utf-8"))
                    except AttributeError:
                        os.remove(f"./songs/songs{YEAR}/{title} by {artist} UNCERTAIN.txt")
                        print(f"    THIS SONG DOES NOT SEEM TO EXIST: {title} by {artist}")
                        with open(f"./songs/songs{YEAR}/{title} by {artist} MISSING.txt", "w") as f:
